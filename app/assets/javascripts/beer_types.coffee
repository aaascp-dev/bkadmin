...
$ ->
  ...
  $('.chips-initial').on "chip.delete", (event, chip) ->
    $('#delete_chip_confirmation_beer_type').val(chip.id)
    $('#delete_chip_confirmation_beer_type').text(chip.tag)
    $('#destroy_beer_type_button').attr("href", "/beer_types/" + chip.id)
